...
public class ChartGeneratorInSelectScene : MonoBehaviour, IChartGenerator
{
    ...
    [SerializeField] private ChartControllerInSelectScene chartController;
    ...
    private void Initialize()
    {
        ...
        {
            Debug.LogWarning("[ChartGeneratorInSelectScene] Timer is not assigned.");
            return;
        }
    ...
    public void Generate(Action callback = null)
    {
        ...
        if (chartController == null || chartController.ChartData == null)
        {
            Debug.LogWarning("[ChartGeneratorInSelectScene] Chart data is null.");
            callback?.Invoke();
            return;
        }
    ...
    private void ClearGeneratedNotes()
    {
    ...
    private static void InitializeFactory(NoteFactoryBinding binding, NoteFactoryInitializingData data)
    {
        ...
        {
            Debug.LogWarning($"[ChartGeneratorInSelectScene] Initialize method not found: {binding.Factory.GetType().Name}");
            return;
        }
    ...
    private static void SpawnEachType(NoteFactoryBinding binding, global::ChartData chartData, INotePositionCalculator positionCalculator, Action<GameObject> onSpawned)
    {
        ...
        if (spawnMethod == null)
        {
            Debug.LogWarning($"[ChartGeneratorInSelectScene] Spawn method not found: {binding.Factory.GetType().Name}");
            return;
        }
        ...
        foreach (INoteData noteData in chartData.GetNoteDataList(binding.NoteType))
        {
            ...
            {
                Debug.LogWarning($"[ChartGeneratorInSelectScene] Spawn failed ({binding.NoteType}): {ex.Message}");
            }
        }
    }
