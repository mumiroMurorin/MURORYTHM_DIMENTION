...
namespace RayFire
{
    ...
    public class RFLimitations
    {
        ...
        public static void Checks (RayfireRigid scr)
        {
            ...
            else if (scr.dmlTp == DemolitionType.Runtime)
            {
                ...
                if (scr.mshDemol.use == true && scr.mshDemol.ch.MultiFrameState == true)
                {
                    ...
                    if (scr.mshDemol.engTp == RayfireShatter.RFEngineType.V1 && scr.mshDemol.sht.clusters.enable == true)
                    {
                        RayfireMan.Log (RFLog.rig_dbgn + scr.name + RFLog.rig_rnt2, scr.gameObject);
                        scr.mshDemol.sht.clusters.enable = false;
                    }
                }
            }
        ...
        // Collision force checks. Uses relativeVelocity. USES FORCED DEMOLITION. 
