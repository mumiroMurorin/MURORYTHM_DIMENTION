...
namespace RayFire
{
    ...
    public class RFEngine
    {
        ...
        static void ProcessShatterFragmentation(RayfireShatter sh) 
        {
            ...
            // Perform slicing ops
            ProcessFragmentation (sh.engine, fragType, combine, element, faceFlt);
        ...
        static List<Transform> CreateShatterFragments(RayfireShatter sh)
        {
            ...
            // Create Unity Mesh objects
            return CreateShatterFragments(sh.engine, sh.material.iMat, inner, planar, size, sh.shell);
        }
        
         // STEP 4.1 Create Unity Mesh objects
        static List<Transform> CreateShatterFragments(RFEngine engine, Material iMat, bool inner, bool planar, float size, RFShell shell)
        {
            // Fragments list
            ...
            for (int i = 0; i < engine.utilFrags.Length; i++)
            {
                ...
                if (engine.skinFlags[i] == false)
                    CreateShatterMeshFragments (engine, groupRoot, i, iMat, inner, planar, size, shell, ref fragments, layer, tag);
                else
        ...
        // Create Mesh Frags
        static void CreateShatterMeshFragments(RFEngine engine, Transform groupRoot, int ind, Material iMat, bool inner, bool planar, float size, RFShell shell, ref List<Transform> fragments, int layer, string tag)
        {
        ...
        static void ProcessRigidFragmentation(RFEngine engine, RayfireRigid rg) 
        {
            ...
            {
                // Perform slicing ops
                ProcessFragmentation (engine, fragType, combine, element, faceFlt);
        ...
        static void ProcessFragmentation(RFEngine engine, FragType fragType, bool combine, int element, int faceFlt) 
        {
            ...
            // Fragment
            engine.utilFrags = Utils.Mesh.Fragment (engine.sliceData, combine, engine.edgeFlags, element * 0.01f, faceFlt, firstPass);
        }
        ...
        IEnumerator RuntimeCachingV2Cor (RayfireRigid scr, bool combine, int element, int faceFlt, bool outCap, bool smooth, int clsCount, int clsLayers, int clsSeed)
        {
            ...
            for (int i = 0; i < batchAmount.Count; i++)
            {
                ...
                // Fragment
                perFrameFrags.Add(Utils.Mesh.Fragment (scr.mshDemol.engine.sliceData, combine, scr.mshDemol.engine.edgeFlags, element * 0.01f, faceFlt, firstPass));
        ...
        static void ComputeMaps(RFEngine engine, Color innerColor, bool smoothInner) 
        {
            for (int i = 0; i < engine.utilFrags.Length; ++i)
                for (int t = 0; t < engine.utilFrags[i].Length; ++t)
                {
                    ...
                    if (engine.skinFlags[i] == false)
                        engine.fragMaps[i][t].ComputeNormals(engine.origMaps[i], engine.utilMeshes[i], engine.utilFrags[i][t], null, null, smoothInner);
                    else
