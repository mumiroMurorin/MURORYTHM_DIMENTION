...
public class StageController_CyberRoom : MonoBehaviour, IStageController
{
    ...
    void IStageController.Initialize(IMusicDataGetter musicDataGetter)
    {
        ...
        {
            Debug.LogWarning("[StageController_CyberRoom] MusicDataGetter is not set.");
            return;
        }
    ...
    string GetDifficultyText(MusicData musicData, Difficulty difficulty)
    {
        ...
        Debug.LogWarning($"[StageController_CyberRoom] Master difficulty text is not set: {symphonyType}");
