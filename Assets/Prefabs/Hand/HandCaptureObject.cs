...
public class HandCaptureObject : MonoBehaviour
{
    ...
    [SerializeField, Min(0f)] float smoothingTime = 0.05f;
    ...
    private void LateUpdate()
    {
        ...
        var currentPosition = (Vector2)handObject.transform.position;
        var smoothedPosition = smoothingTime > 0f
            ? Vector2.SmoothDamp(
                currentPosition,
                targetPosition,
                ref smoothVelocity,
        ...
        ApplyHandPosition(smoothedPosition);
    }

    private void ApplyHandPosition(Vector2 pos)
    {
        handObject.transform.position = new Vector3(pos.x, pos.y, handObject.transform.position.z);

        // x^2 + y^2 = r^2
        float scaleX = Mathf.Sqrt(Mathf.Max(0f, radius * radius - pos.y * pos.y)) * 2f;
        float dy = Mathf.Sqrt(Mathf.Max(0f, radius * radius - pos.x * pos.x));
