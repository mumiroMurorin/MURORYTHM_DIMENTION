...
public class HandCaptureObject : MonoBehaviour
{
    ...
    public void OnMoveHandPosition(Vector2 pos)
    {
        ...
        // x^2 + y^2 = r^2
        float scaleX = Mathf.Abs(Mathf.Sqrt(Mathf.Pow(radius, 2) - Mathf.Pow(pos.y, 2))) * 2f;
        float scaleY = Mathf.Abs(Mathf.Sqrt(Mathf.Pow(radius, 2) - Mathf.Pow(pos.x, 2)));
        ...
        verticalLineObj.transform.position = new Vector3(pos.x, -scaleY / 2f, verticalLineObj.transform.position.z);
    }
}
