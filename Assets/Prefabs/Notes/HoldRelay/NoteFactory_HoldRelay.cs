...
public class NoteFactory_HoldRelay : NoteFactory<NoteData_HoldRelay>
{
    ...
    public override void Initialize(NoteFactoryInitializingData initializingData)
    {
        ...
        judgementSettings = initializingData.GetJudgementSettings<NoteJudgementConfig>(NoteType.HoldRelay);
    }

    public override NoteObject<NoteData_HoldRelay> Spawn(NoteData_HoldRelay data, INotePositionCalculator positionCalculator)
    {
        // 生成
        NoteObject<NoteData_HoldRelay> note = GenerateNoteInstance(ConvertNoteData(data));
    ...
    private NoteData_HoldRelay ConvertNoteData(NoteData_HoldRelay data)
    {
        // ノーツデータにいろいろ追加
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    private NoteObject<NoteData_HoldRelay> GenerateNoteInstance(NoteData_HoldRelay data)
    {
        ...
        // コンポーネントを取得
        NoteObject<NoteData_HoldRelay> note = origin.GetComponent<NoteObject<NoteData_HoldRelay>>();
    ...
    /// 位置調整など
    /// </summary>
    private void SetTransform(NoteObject<NoteData_HoldRelay> note, float spawnZ)
    {
        // 動く地面を親登録
