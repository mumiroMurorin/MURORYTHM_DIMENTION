...
public class NoteFactory_HoldStart : NoteFactory<NoteData_HoldStart>
{
    ...
    public override void Initialize(NoteFactoryInitializingData initializingData)
    {
        ...
        judgementSettings = initializingData.GetJudgementSettings<NoteJudgementConfig>(NoteType.HoldStart);
    }

    public override NoteObject<NoteData_HoldStart> Spawn(NoteData_HoldStart data, INotePositionCalculator positionCalculator)
    {
        // 生成
        NoteObject<NoteData_HoldStart> note = GenerateNoteInstance(ConvertNoteData(data));
    ...
    private NoteData_HoldStart ConvertNoteData(NoteData_HoldStart data)
    {
        // ノーツデータにいろいろ追加
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    private NoteObject<NoteData_HoldStart> GenerateNoteInstance(NoteData_HoldStart data)
    {
        ...
        // コンポーネントを取得
        NoteObject<NoteData_HoldStart> note = origin.GetComponent<NoteObject<NoteData_HoldStart>>();
    ...
    /// 位置調整など
    /// </summary>
    private void SetTransform(NoteObject<NoteData_HoldStart> note, float spawnZ)
    {
        // 動く地面を親登録
