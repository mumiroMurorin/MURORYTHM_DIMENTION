...
public class NoteFactory_DynamicGroundLeftward : NoteFactory<NoteData_DynamicGroundLeftward>
{
    [SerializeField] GameObject noteObjectOriginPrefab;
    DynamicNoteJudgementConfig judgementSettings;
    ...
    public override void Initialize(NoteFactoryInitializingData initializingData)
    {
        ...
        judgementSettings = initializingData.GetJudgementSettings<DynamicNoteJudgementConfig>(NoteType.DynamicGroundLeftward);
    }

    public override NoteObject<NoteData_DynamicGroundLeftward> Spawn(NoteData_DynamicGroundLeftward data, INotePositionCalculator positionCalculator)
    {
        // 生成
        NoteObject<NoteData_DynamicGroundLeftward> note = GenerateNoteInstance(ConvertNoteData(data));
    ...
    private NoteData_DynamicGroundLeftward ConvertNoteData(NoteData_DynamicGroundLeftward data)
    {
        // ノーツデータにいろいろ追加
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    private NoteObject<NoteData_DynamicGroundLeftward> GenerateNoteInstance(NoteData_DynamicGroundLeftward data)
    {
        ...
        // コンポーネントを取得
        NoteObject<NoteData_DynamicGroundLeftward> note = origin.GetComponent<NoteObject<NoteData_DynamicGroundLeftward>>();
    ...
    private GameObject GenerateNoteObject(int size)
    {
        ...
        {
            // ※ポジションと角度の計算
            //pos = new Vector3(10 * Mathf.Cos((((size - 1) / 2f - (size - i - 1)) * 11.25f - 90f) * Mathf.Deg2Rad), 10 * Mathf.Sin((((size - 1) / 2f - (size - i - 1)) * 11.25f - 90f) * Mathf.Deg2Rad), 0);
    ...
    /// 位置調整など
    /// </summary>
    private void SetTransform(NoteObject<NoteData_DynamicGroundLeftward> note, float spawnZ)
    {
        // 動く地面を親登録
