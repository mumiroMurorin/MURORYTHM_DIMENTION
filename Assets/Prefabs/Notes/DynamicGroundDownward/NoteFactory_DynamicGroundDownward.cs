...
public class NoteFactory_DynamicGroundDownward : NoteFactory<NoteData_DynamicGroundDownward>
{
    ...
    public override void Initialize(NoteFactoryInitializingData initializingData)
    {
        ...
        judgementSettings = initializingData.GetJudgementSettings<DynamicNoteJudgementConfig>(NoteType.DynamicGroundDownward);
    }

    public override NoteObject<NoteData_DynamicGroundDownward> Spawn(NoteData_DynamicGroundDownward data, INotePositionCalculator positionCalculator)
    {
        // 生成
        NoteObject<NoteData_DynamicGroundDownward> note = GenerateNoteInstance(ConvertNoteData(data));
    ...
    private NoteData_DynamicGroundDownward ConvertNoteData(NoteData_DynamicGroundDownward data)
    {
        // ノーツデータにいろいろ追加
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    private NoteObject<NoteData_DynamicGroundDownward> GenerateNoteInstance(NoteData_DynamicGroundDownward data)
    {
        ...
        // コンポーネントを取得
        NoteObject<NoteData_DynamicGroundDownward> note = origin.GetComponent<NoteObject<NoteData_DynamicGroundDownward>>();
    ...
    /// 位置調整など
    /// </summary>
    private void SetTransform(NoteObject<NoteData_DynamicGroundDownward> note, float spawnZ)
    {
        // 動く地面を親登録
