...
{
    public class BodytrackingAddLayer : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            {
                Debug.Log("Loading Lite Pose model...");
            ...
            {
                Debug.Log("Loading Full Pose model...");
            ...
            while (true)
            {
                ...
                if (poseLandmarksStream.TryGetNext(out var LandMarks))
                {
                    ...
                    if(_annotationController == null) { Debug.LogError("_annotationController is null, unable to draw landmarks."); break; }

                    _annotationController.DrawLandMark(new List<NormalizedLandmarkList> { LandMarks });
                    _annotationController.SetActiveLandMarks(true);
                }
                else
                {
                    Debug.LogWarning("No pose landmarks received.");
                    //_annotationController.SetActiveLandMarks(false);
                }
            }
        }
